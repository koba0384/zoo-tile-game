...
def render_tile_html(tile, meeple=None, size=84):
    edges = tile["edges"]
    border = lambda edge: "6px solid #111827" if edge == "fence" else "1px dashed #d1d5db"
    ...
    if meeple is not None:
        ...
        meeple_html = f"""
        <div style='position:absolute; top:4px; right:6px; font-size:12px; background:{color};
             color:white; padding:1px 6px; border-radius:999px; line-height:1.3;'>
             {label}
        </div>
        """
    ...
    <div style="
        ...
        position:relative; font-size:34px; background:#ffffff;
...
def render_board_html(board):
    ...
    for y in range(min_y, max_y + 1):
        ...
        for x in range(min_x, max_x + 1):
            ...
            if coord in board:
                cell = board[coord]
                tile_html = render_tile_html(cell["tile"], cell.get("meeple"), size=58)
                cells.append(f"""
                <div style='display:flex; flex-direction:column; align-items:center; gap:4px;'>
                    {tile_html}
                    <div style='font-size:11px; color:#6b7280;'>{x},{y}</div>
                </div>
                """)
            else:
                cells.append(f"""
                <div style='display:flex; flex-direction:column; align-items:center; gap:4px;'>
                    <div style="
                        width:58px; height:58px; border:1px dashed #e5e7eb;
                        border-radius:10px; background:#fafafa; margin:auto;">
                    </div>
                    <div style='font-size:11px; color:#d1d5db;'>{x},{y}</div>
                </div>
                """)
        row_html = "".join(
            [f"<div style='display:grid; place-items:center; width:72px;'>{c}</div>" for c in cells]
        )
        rows.append(f"<div style='display:flex; gap:6px; margin-bottom:8px;'>{row_html}</div>")

    return "<div style='overflow-x:auto; padding-bottom:8px;'>" + "".join(rows) + "</div>"
...
def render_scoring_panel():
    ...
    if not last:
        ...
        return
    st.subheader(f"直近の得点: {last['title']}")
...
def render_sidebar():
    st.sidebar.title("どうぶつえんタイルゲーム")
    default_players = st.session_state.get("num_players", 2)
    num_players = st.sidebar.selectbox("プレイヤー人数", [2, 3, 4], index=max(0, default_players - 2))
    if st.sidebar.button("新しいゲーム", use_container_width=True):
        start_game(num_players)
        st.rerun()

    st.sidebar.caption("ローカル対戦用の試作版です。")
    st.sidebar.markdown(
        """
- 手番ではタイルを1枚置く
...
def main():
    ...
    st.caption("GitHub / Streamlit 用の雛形。盤面・多数決・囲い完成・基本得点・コンボ・内包ボーナスまで試せます。")

    left, center, right = st.columns([1.1, 1.4, 1.1], gap="large")
    ...
    with left:
        ...
        if st.session_state.game_over:
            ...
            st.markdown(
                f"<div style='padding:10px 14px; border-radius:12px; color:white; background:{PLAYER_COLORS[player]}; font-weight:700;'>"
                f"{PLAYER_LABELS[player]}プレイヤーの番</div>",
                unsafe_allow_html=True,
            )

        st.write(f"山札残り: **{len(st.session_state.deck)}枚**")
        st.subheader("スコア")
        score_table = {
    ...
    with right:
        ...
        if st.session_state.current_tile is None:
            ...
            st.markdown(render_tile_html(tile, size=96), unsafe_allow_html=True)
            ...
            if valid_positions:
                ...
                st.warning("この向きでは置ける場所がありません。回転を変えてください。")
